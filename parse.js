...
var fs = require('fs')
    json = require(path),
    URL_BASE = 'http://www.videourl.com/video?t=',
    items = [];

/*
* Create INSERT Statement
* INSERT clips (clipid, clipname, title, credit, synopsis) values (null, "string", "string", "strign", "string");
* INSERT words (clipid, text, start, end) values (, "string", float, float);
*/

var sqlQuery = 'INSERT clips (clipid, clipname, title, credit, synopsis) '
             + 'values ('
             + id + ', '
             + '"' + filename + '",'
             + '"Young George Michael",'
             + '"BBC",'
             + '"Whether you\'re a fan of George Michael - or not - nobody can deny his brilliance as a performer and artist. Now, clues to his very early songwriting talent has been discovered too - by an old friend from primary school."'
             +');\n';

function createQuery(words, index, speaker){
    sqlQuery += 'INSERT words (clipid, text, intime, outtime, speakerid) '
             + 'values ('
             + id + ', '
             + '"' + words[index].text + '",'
             + words[index].start + ','
...
/*
* Goes through each segment words and creates an article
*/
for (var i=0; i < json.segments.length; i++){
    for (var j=0; j < json.segments[i].words.length; j++){
        //if (json.segments[i].words[j].text.length > 3) {
            createQuery(json.segments[i].words, j, json.segments[i].speaker)
        //}
    }
}

/*
* Write items to output.txt file
*/
fs.writeFileSync('output.txt', sqlQuery);
